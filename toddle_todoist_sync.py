...
Reads events from the Toddle calendar's private ICS feed, classifies each one
into the right Todoist section / subject label / type label / priority,
optionally rewrites vague Toddle titles into short action-item task names via
the Gemini API, and creates the task -- skipping anything already synced by
checking whether the Toddle share link already appears in an existing task's
description.
...
before running):
    ...
    GEMINI_API_KEY      - Optional. If unset, titles fall back to a plain
                           verb-based rewrite instead of an AI rewrite.
...
# IMPORTANT: these keys must match the "Classes:" field of your Toddle
# calendar events EXACTLY. Toddle sometimes appends a period/section number
# to the class name (e.g. "Life Skills 7" instead of just "Life Skills").
# The names below are guessed from your class schedule screenshot -- check
# one real Toddle event's description after your first run and fix any
# mismatches here, or the event will silently fall through to GENERAL_SECTION
# with no subject label.
CLASS_MAP = {
    "Computer Science I 4": ("6hW6Cqp494CVWQVq", "coding"),
    "Integrated Math II with Trigonometry 2": ("6hW6CqpVF44VvCQq", "math"),
    "Chemistry 4": ("6hW6CqmjHr226JQq", "science"),
    "French: Intermediate 2": ("6hW6CqpF9pfmM78q", "french"),
    "Life Skills 3": ("6hW6Cqq9P6p7WWfq", "life skills"),
    "English 10/American History 1": ("6hW6CqqP8Pc62qhH", "american studies"),
    # "HS Tech" showed up in your Toddle feed but isn't mapped yet -- it'll
    # land in General with no subject label until you tell me what it is.
}
GENERAL_SECTION = "6hW6Cqm4x6ggJfPH"  # catch-all for classes not in the map

# Classes where regular (non-assessment) work runs P2 instead of P3.
WORRY_CLASSES = {"Integrated Math II with Trigonometry 2", "English 10/American History 1"}
...
def build_description(title: str, link: str, raw_description: str) -> str:
    """Task description shown in Todoist: a clickable link to submit the
    assignment on Toddle, followed by the full original Toddle description
    (instructions, rubric notes, etc.) so nothing gets lost.

    The link text is the (unique) event title rather than something generic
    like "Submit on Toddle" -- signature_keys() also picks up markdown link
    text as a fallback dedup key, and a generic shared string across every
    task would collide between unrelated assignments.
    """
...
def signature_keys(content: str, description: str) -> set:
    ...
    descriptions that are nothing else -- some tasks may have explanatory
    prose before the link, which a full-string-only match would silently
    miss (this is also why title-only fallback matching can never recover a
    title Todoist has already overwritten via its own link-preview
    auto-linkification -- there's nothing here that can fix a description
    that's already lost the original wording, only avoid missing one that
    hasn't).
    """
...
def classify(task_type: str, class_name: str, title: str, description: str):
    ...
    if is_assessment:
        ...
        elif re.search(r"\btest\b", text) or "lab assessment" in text:
            type_label = "test"
        else:
...
def completed_task_keys(since: str = "2026-08-01") -> set:
    ...
    active_tasks_by_key(), and that endpoint responds with
    "items"/"nextCursor" (camelCase), not "results"/"next_cursor" like the
    regular task list does.
    """
    keys = set()
    cursor = None
...
def update_task(task_id: str, **fields):
    """Update ordinary task fields (due date, labels, priority).

    NOTE: section_id is deliberately NOT accepted here. Todoist's regular
    task-update endpoint silently ignores section_id -- it does nothing and
    returns success, which is why earlier runs logged a section_id "update"
    that never actually moved the task. Moving sections requires the
    ...
    if fields:
        ...
        resp.raise_for_status()
    if section_id:
        move_task_section(task_id, section_id)


def move_task_section(task_id: str, section_id: str):
    resp = requests.post(
        f"{TODOIST_API}/{task_id}/move",
        headers=HEADERS,
...
def main():
    ...
    for component in cal.walk("VEVENT"):
        ...
        desc = build_description(title, link, description)
